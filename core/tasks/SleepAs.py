...
def import_SleepAs_csv(filename='/Apps/Sleep Cloud Backup/Sleep as Android Data'):
    logger.info('Running the SleepAs import')
    if default_storage.exists(filename):
        logger.info('Processing file: "%s"' % filename)
        data = read_csv(filename)
        # self.stdout.write(self.style.SUCCESS('data : %s' % data))
        logger.info('converted file: "%s"' % filename)
        import_data(data)
        logger.info('imported file: "%s"' % filename)
